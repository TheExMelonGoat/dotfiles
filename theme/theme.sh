...
## Pywal Theme with Alphabetical Wallpaper Cycling
source_pywal() {
	...
	generate_colors() {
		...
		if command -v wal &>/dev/null; then
			...
			notify-send -t 5000 -h string:x-canonical-private-synchronous:sys-notify-runpywal -i ${PATH_MAKO}/icons/timer.png "Generating Colorscheme from $(basename "$IMG")..."
			wal -q -n -s -t -e -i "$IMG"
